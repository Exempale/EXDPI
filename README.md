...
<table>
  <tr>
    ...
    <td align="center"><b>Полночь</b><br><img src="docs/screenshots/main-midnight.png" width="280"></td>
  </tr>
</table>

Главное окно: большой переключатель ON/OFF, индикатор состояния, готовая
`mtproto`-ссылка для одного клика и текущий режим работы. Иконка «полумесяц»
в углу переключает тему по кругу (тёмная → светлая → полночь), шестерёнка
ведёт в настройки.
...
Тему можно переключить и из настроек (Тёмная / Светлая / Полночь). Применяется
сразу после сохранения — без перезапуска приложения.
...
пошаговую инструкцию для Telegram Desktop, телефонов и голосовых чатов (VC) —
с готовыми параметрами вашего прокси и кнопкой «скопировать `tg://proxy`».
...
5. Кнопка-иконка «полумесяц» рядом с шестерёнкой циклически переключает тему.
...
1. Откройте EXDPI на компьютере, скопируйте `tg://proxy`-ссылку.
2. В настройках EXDPI замените `proxy_host=127.0.0.1` на локальный IP компьютера
   (например `192.168.0.5`) — телефон должен видеть его в одной сети.
3. Отправьте ссылку себе в Saved Messages → тапните по ней в мобильном
   Telegram → «Подключиться к прокси».
...
|--------|----------|------|
| Свой набор | Ваш ручной список — не перезаписывается. | — |
| ИИ-сервисы (дефолт) | ChatGPT, Claude, Devin, Gemini, Grok, Perplexity, HuggingFace. | `app/config.py: DEFAULT_CUSTOM_DOMAINS` |
...
| **Полночь** | Глубокий тёмно-синий с холодным акцентом. |

Переключается:
* Иконкой «полумесяц» в шапке главного окна — циклически.
...
├── app/
│   ├── theme.py            # Темы (dark/light/midnight) + apply_theme()
