...
/* EXDPI: позволяет JNI закрыть слушающий сокет извне и таким образом
 * прервать event_loop. Хранит srvfd текущего запуска (-1 если простоя). */
volatile int byedpi_srvfd_global = -1;
...
int run(struct sockaddr_ina *srv)
{
    ...
    byedpi_srvfd_global = fd;
    int rc = event_loop(fd);
    byedpi_srvfd_global = -1;
    return rc;
}
