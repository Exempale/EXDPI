...
/* EXDPI: вынесли entrypoint в отдельную функцию, чтобы вызывать из JNI.
 * При повторном вызове сбрасываем глобальное состояние params + getopt. */
extern int optind;

static void byedpi_reset_globals(void)
{
    optind = 1;
    /* params — статическая структура: пересчитаем дефолты, чтобы повторный
     * вызов byedpi_main не получил мусор от предыдущего запуска. */
    ...
    params.laddr.sin6_family = AF_INET;
}

int byedpi_main(int argc, char **argv)
{
    byedpi_reset_globals();
    #ifdef _WIN32
    WSADATA wsa;
